...
function GenomeListController($scope,$http,$timeout){$scope.orderProp='id';$scope.query='';$scope.curPos=0;$scope.pageSize=20;$scope.delayPromise=undefined;function fetch(){var url='/edge/genomes/?q='+encodeURIComponent($scope.query)+'&s='+$scope.curPos+'&p='+$scope.pageSize;$http.get(url).success(function(data){$scope.genomes=data;});}
$scope.nextPage=function(){$scope.curPos+=$scope.pageSize;fetch();};$scope.prevPage=function(){$scope.curPos-=$scope.pageSize;if($scope.curPos<0){$scope.curPos=0;}
fetch();};$scope.delayFetch=function(){if($scope.delayPromise!==undefined){$timeout.cancel($scope.delayPromise);$scope.delayPromise=undefined;}
$scope.delayPromise=$timeout(function(){fetch();$scope.delayPromise=undefined;},300);};fetch();}
...
function GenomeFragmentController($scope,$routeParams,$injector,$http){$injector.invoke(FragmentController,this,{$scope:$scope});$scope.genomeId=$routeParams.genomeId;$scope.genome=undefined;$scope.changes_and_locs=[];$http.get('/edge/genomes/'+$scope.genomeId+'/').success(function(genome){$scope.genome=genome;genome['fragments'].forEach(function(fragment){if(fragment['id']==$scope.fragmentId){if(fragment['changes']){fragment['changes'].forEach(function(changed_loc){edgeFetchChanges($http,fragment['id'],changed_loc,function(desc,annotation){$scope.changes_and_locs.push({'desc':desc,'annotation':annotation});});});}}});});}
