...
function GenomeFragmentController($scope,$routeParams,$injector,$http){$injector.invoke(FragmentController,this,{$scope:$scope});$scope.genomeId=$routeParams.genomeId;$scope.genome=undefined;$scope.changes_and_locs=[];function fetchGenome(){$http.get('/edge/genomes/'+$scope.genomeId+'/').success(function(genome){$scope.genome=genome;genome['fragments'].forEach(function(fragment){if(fragment['id']==$scope.fragmentId){if(fragment['changes']){fragment['changes'].forEach(function(changed_loc){edgeFetchChanges($http,fragment['id'],changed_loc,function(desc,annotation){$scope.changes_and_locs.push({'desc':desc,'annotation':annotation});});});}}});});}
